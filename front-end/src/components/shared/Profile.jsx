...
export const  Profile = () => {
    ...
    const getUserData = async () =>{
        setLoading(true);
        const response = await getAuthenticatedUserData(localStorage.getItem('token'));
        setLoading(false);
    ...
    const handleSubmit = async (e) =>{
        ...
        try{
            ...
            const response = await editProfileData(localStorage.getItem("token"),data);
    ...
    const getUserStores = async (page) =>{
        const response = await getUserStoresList(localStorage.getItem('token'),page);
    ...
    <div>
      ...
      <div className="lg:ml-[21%] px-2">
        <div className="pt-6 w-[100%] flex justify-between">
          ...
          <Button text={'Edit profile Data'} onClick={() => setOpenEditProfile(true)} width={"20%"}/>
        </div>
        ...
        <div className="mt-6">
            {
                !loading && <div>
                        <div className="flex gap-4 items-center">
                            <div>
                                <img src={userData.profile_picture} className="w-20 h-20 rounded-full"/>
                            </div>
                            <div className="flex justify-start flex-col">
                                ...
                                <span className="font-semibold text-gray-700">Joined at {moment(userData.created_at).format("DD-MM-YYYY")}</span>
                            </div>
                        </div>
                        <div className="mt-2">
                        ...
                        <div className="mt-5">
                            ...
                            <div className="flex flex-wrap mt-5 justify-between lg:gap-5">
                                ...
                                <div className="w-[48%] bg-dark border-2 border-gray-700 rounded-sm border-dashed cursor-pointer flex justify-center items-center" onClick={() => getMore()}>
        ...
        {
            openEditProfile && <EditProfile userData={userData} loading={editLoading} handleChangeProfilePicture={handleChangeProfilePicture} handleSubmit={handleSubmit} handleChange={handleChange} setOpenEditProfile={setOpenEditProfile} />
        }
        {
