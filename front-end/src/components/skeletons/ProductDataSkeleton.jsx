
export const ProductDataSkeleton = () => {
  return (
    <div>
        <div role="status" className="space-y-8 w-[100%] animate-pulse md:space-y-0 md:space-x-8 rtl:space-x-reverse md:flex">
            <div className="flex items-center justify-center w-full h-96 bg-gray-300 rounded sm:w-[70%] dark:bg-gray-400">
                <svg className="w-10 h-10 text-gray-200 dark:text-gray-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                    <path d="M18 0H2a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2Zm-5.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm4.376 10.481A1 1 0 0 1 16 15H4a1 1 0 0 1-.895-1.447l3.5-7A1 1 0 0 1 7.468 6a.965.965 0 0 1 .9.5l2.775 4.757 1.546-1.887a1 1 0 0 1 1.618.1l2.541 4a1 1 0 0 1 .028 1.011Z"/>
                </svg>
            </div>
            <div className="w-full">
                <div className="flex flex-row gap-2 items-center">
                    <div className="rounded-full bg-slate-400 h-10 w-10"></div>
                    <div className="h-2 bg-slate-400 rounded w-[30%]"></div>
                </div>
                <div className="mt-6">
                    <div className="h-2.5 bg-gray-200 rounded-full dark:bg-gray-400 w-48 mb-4"></div>
                    <div className="h-4 bg-gray-200 rounded-full dark:bg-gray-400 w-20 mb-4"></div>
                    <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[480px] mb-2.5"></div>
                    <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 mb-2.5"></div>
                    <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[440px] mb-2.5"></div>
                    <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[460px] mb-2.5"></div>
                    <div className="mt-5 flex flex-col gap-2">
                        <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[360px]"></div>
                        <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[360px]"></div>
                        <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[360px]"></div>
                        <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[360px]"></div>
                    </div>
                    <div className="mt-4 flex flex-col gap-1">
                        <div className="h-8 bg-gray-200 rounded-md dark:bg-gray-400 max-w-[100%] mb-2.5"></div>
                        <div className="h-8 bg-gray-200 rounded-md dark:bg-gray-400 max-w-[100%] mb-2.5"></div>
                    </div>
                </div>
            </div>
        </div>
        <div className="mt-6 animate-pulse flex flex-col gap-2">
            <div>
                <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 mb-2.5"></div>
                <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[440px] mb-2.5"></div>
            </div>
            <div>
                <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 mb-2.5"></div>
                <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[440px] mb-2.5"></div>
            </div>
            <div>
                <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 mb-2.5"></div>
                <div className="h-2 bg-gray-200 rounded-full dark:bg-gray-400 max-w-[440px] mb-2.5"></div>
            </div>
        </div>
    </div>
    )
}
