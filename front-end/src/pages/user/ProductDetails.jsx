...
import { getProductLikes, isProductLiked, likeAProduct } from "../../services/likeServices";
...
export const ProductDetails = () => {
  ...
  useEffect(() =>{
    const checkIfProductLiked = async () => {
      const response = await isProductLiked(localStorage.getItem("token"),id);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const response2 = await postMessage(
          localStorage.getItem("token"),
          messageData
        );
              ...
              <div className="md:w-[58%]">
                <div className="flex justify-between items-center mt-2 md:mt-0">
                  <div className="flex gap-2 items-center">
                    <div>
                      <img
                        src={productDetails.store.user.profile_picture}
                        className="w-10 h-10 rounded-full"
                      />
                    </div>
                    <div className="flex flex-col">
                      <span className="font-semibold hover:text-blue-500 cursor-pointer duration-200">
                        <Link
                          to={`/user/userData/${productDetails.store.user.id}`}
                        >
                          {productDetails.store.user.fullName}
                        </Link>
                      </span>
                      <span className="text-sm font-semibold text-gray-600">
                        ...
                        <span className="text-blue-500 cursor-pointer">
                          <Link
                            to={`/store/storeData/${productDetails.store.id}`}
                          >
                            {productDetails.store.storeName}
                          </Link>
                        </span>{" "}
                        store
                      </span>
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-col gap-1 text-black">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        />
                        <span className="font-semibold">
                          {productComments.length} Commented on this product
                        </span>
                      </div>
                      <div className="flex gap-2 items-center">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        />
                        <span className="font-semibold">
                          {productDetails.delivry}
                        </span>
                      </div>
                      <div className="flex gap-2 items-center">
                        <ClockIcon className="w-5 h-5" strokeWidth={"2"} />
                        <span className="font-semibold">
                          Posted {moment(productDetails.created_at).fromNow()}
                        </span>
                      </div>
                    </div>
                    <div className="mt-2 flex flex-col gap-2">
                      <Button
                        text={"Is that item available?"}
                        onClick={sendMessage}
                        loading={messageLoading}
                      />
                      <Button
                        text={"Comment on this product"}
                        bg={"bg-blue-500"}
                        onClick={() => setOpenPostComment(true)}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col mt-3">
                {productComments && productComments.length ? (
                  productComments.map((comment) => {
                    return (
                      <div className="mb-5 bg-gray-100 py-4 px-3 rounded-xl">
                        <div className="flex justify-between items-center gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span
                              className="font-semibold cursor-pointer hover:text-sky-500 duration-200"
                              onClick={() =>
                                navigate(
                                  `/user/userData/${comment.commenterId}`
                                )
                              }
                            >
                  ...
                  <p className="font-semibold text-center">
                    No comments!, be the first one to comment On this product
                  </p>
                )}
              </div>
            </div>
